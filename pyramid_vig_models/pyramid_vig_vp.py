...
class PadPrompter(nn.Module):
    def __init__(self, crop_size, prompt_size):
...
class DeepGCN(torch.nn.Module):
    def __init__(self, opt):
        ...
        self.visual_prompts = PadPrompter(224, 32)
        self.model_init()
    ...
    def forward(self, inputs):
        inputs = self.visual_prompts(inputs)
...
@register_model
def pvig_vp_ti_224_gelu(pretrained=False, **kwargs):
    class OptInit:
...
@register_model
def pvig_vp_s_224_gelu(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=1000, drop_path_rate=0.0, **kwargs):
            self.k = 9 # neighbor num (default:9)
            self.conv = 'mr' # graph conv layer {edge, mr}
...
@register_model
def pvig_vp_m_224_gelu(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=1000, drop_path_rate=0.0, **kwargs):
            self.k = 9 # neighbor num (default:9)
            self.conv = 'mr' # graph conv layer {edge, mr}
...
@register_model
def pvig_vp_b_224_gelu(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=1000, drop_path_rate=0.0, **kwargs):
            self.k = 9 # neighbor num (default:9)
            self.conv = 'mr' # graph conv layer {edge, mr}
