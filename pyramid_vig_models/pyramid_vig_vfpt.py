...
class Grapher(nn.Module):
    ...
    def forward(self, x):
        ...
        node_prompts = self.prompt_dropout(node_prompts.expand([B,-1,-1]))
        node_prompts = self.FT(node_prompts)[:,:,None,:]
...
@register_model
def pvig_vfpt_ti_224_gelu(pretrained=False, **kwargs):
    class OptInit:
...
@register_model
def pvig_vfpt_s_224_gelu(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=1000, drop_path_rate=0.0, **kwargs):
            self.k = 9 # neighbor num (default:9)
            self.conv = 'mr' # graph conv layer {edge, mr}
...
@register_model
def pvig_vfpt_m_224_gelu(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=1000, drop_path_rate=0.0, **kwargs):
            self.k = 9 # neighbor num (default:9)
            self.conv = 'mr' # graph conv layer {edge, mr}
...
@register_model
def pvig_vfpt_b_224_gelu(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=1000, drop_path_rate=0.0, **kwargs):
            self.k = 9 # neighbor num (default:9)
            self.conv = 'mr' # graph conv layer {edge, mr}
