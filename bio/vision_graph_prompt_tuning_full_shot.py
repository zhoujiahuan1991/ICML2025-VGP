...
from vgp_model import GNN_visiongraphprompt
...
def summary_parameters(model=None, in_detail=True):
    if in_detail:
...
def main():
    ...
    parser.add_argument('--model_file', type=str, default = 'pretrained_models/edgepred.pth', help='File path to read the model (if there is any)')
    parser.add_argument('--dataset', type=str, default = 'ppi', help='The dataset name')
    parser.add_argument('--exp_name', type=str, default = 'debug', help='Experiment name')
    parser.add_argument('--gnn_type', type=str, default="gin")
    parser.add_argument('--tuning_type', type=str, default="vgp", help='gpf for GPF and gpf-plus for GPF-plus in the paper')
    ...
    print(dataset)
    exp_dir=f'{args.tuning_type}-{args.dataset}/{args.exp_name}'
    os.makedirs(exp_dir, exist_ok=True)#exp_name
    import shutil
    shutil.copy('vgp_model.py', exp_dir)
    shutil.copy('vision_graph_prompt_tuning_full_shot.py', exp_dir)
    log_file = f'{exp_dir}/fullshot-result-lr{args.lr}-head{args.num_layers}.log'
    ...
    #set up model  
    model = GNN_visiongraphprompt(args.num_layer, args.emb_dim, num_tasks, JK = args.JK, drop_ratio = args.dropout_ratio, graph_pooling = args.graph_pooling, gnn_type = args.gnn_type, layer_number = args.num_layers)

    # #                                      num_tasks
    # model = GNN_graphpred(args.num_layer, args.emb_dim, 40, JK = args.JK, drop_ratio = args.dropout_ratio, graph_pooling = args.graph_pooling, gnn_type = args.gnn_type, layer_number = args.num_layers)
    ...
    peft_list = ['graph_pred_linear', 'graph_prompt', 'node_prompter', 'edge_prompt']
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if args.split == "random":
            ...
            test_acc = np.mean(eval(args, model, device, test_loader)).item()
            
            test_acc_easy = eval(args, model, device, test_easy_loader)
            test_acc_hard = eval(args, model, device, test_hard_loader)
            with open(log_file, 'a+') as f:
                f.write(f'Epoch {epoch}:\n')
                f.write(f'easy split mean is {np.mean(test_acc_easy).item()*100}\n')
                f.write(f'easy split std is {np.std(test_acc_easy).item()*100}\n')
                f.write(f'hard split mean is {np.mean(test_acc_hard).item()*100}\n')
                f.write(f'hard split std is {np.std(test_acc_hard).item()*100}\n')
                f.write(f'Avearage accuracy accross species is {test_acc*100}\n')
                if test_acc > best_test_acc:
                    best_test_acc = test_acc
                    f.write(f"==========Best Test Acc {round(best_test_acc*100,2)}%==========\n")
                    save_path = f"{exp_dir}/fullshot-{args.tuning_type}-head{args.num_layers}-lr{args.lr}-best.pth"
                    torch.save({
                        'epoch': epoch,
            ...
            test_acc_easy_mean.append(np.mean(test_acc_easy))
            test_acc_easy_std.append(np.std(test_acc_easy))
            test_acc_hard_mean.append(np.mean(test_acc_hard))
            test_acc_hard_std.append(np.std(test_acc_hard))

            print(test_acc_easy)
            print(test_acc_hard)

        print("")
    best_epoch_easy=np.array(test_acc_easy_mean).argmax()
    best_epoch_hard=np.array(test_acc_hard_mean).argmax()
    with open(log_file, 'a+') as f:
        ...
        f.write('best_epoch_easy='+str(best_epoch_easy)+'\n')
        f.write('best_epoch_hard='+str(best_epoch_hard)+'\n')
        f.write('Best test_acc_easy_list='+str(np.array(test_acc_easy_list)[best_epoch_easy])+'\n'+'Best test_acc_hard_list='+str(np.array(test_acc_hard_list)[best_epoch_hard])+'\n')
        f.write('runseed '+str(args.runseed)+'\n'+'Val-Best test_acc_easy_list='+str(np.array(test_acc_easy_list)[np.array(val_acc_list).mean(axis=1).argmax()])+'\n'+'Val-Best test_acc_hard_list='+str(np.array(test_acc_hard_list)[np.array(val_acc_list).mean(axis=1).argmax()])+'\n')
        f.write('\n')
