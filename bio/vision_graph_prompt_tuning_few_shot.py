...
def main():
    ...
    test_acc_hard_mean = []
    test_acc_hard_std = []
    with open(f'{args.tuning_type}-fewshot-result.log', 'a+') as f:
        f.write(f'\n\n====================================')
    for epoch in range(1, args.epochs+1):
        ...
        if args.split == "random":
            ...
            with open(f'{args.tuning_type}-fewshot-result.log', 'a+') as f:
                ...
                f.write(f'mean(test_acc_easy){np.mean(test_acc_easy).item()}\n')
                f.write(f'std(test_acc_easy){np.std(test_acc_easy).item()}\n')
                f.write(f'mean(test_acc_hard){np.mean(test_acc_hard).item()}\n')
                f.write(f'std(test_acc_hard){np.std(test_acc_hard).item()}\n')
