...
class JSONDataset(torch.utils.data.Dataset):
    def __init__(self, data_path, split, dataset_name, transform, dataset_perc=1.0):
        assert split in {"train", "val", "test"}, "Split '{}' not supported for {} dataset".format(split, dataset_name)
        self._split = split
    ...
    def _construct_imdb(self):
        ...
        self.classes = self._class_id_cont_id
    ...
    def __getitem__(self, index):
        ...
        # raw_img = self.image_transform(im)
        label = self._imdb[index]["class"]
        im = self.transform(im)
        # return im, label, (self._imdb[index]["im_path"], raw_img)
        return im, label
...
class CUB200Dataset(JSONDataset):
    ...
    def __init__(self, root, split, transform):
        ...
        data_path = os.path.join(root, base_folder)
