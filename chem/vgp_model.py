...
class CustomMessagePassing(torch.nn.Module):
    ...
    def propagate(self, edge_index, size=None, **kwargs):
        ...
        message_out = self.message(x[col], edge_attr=kwargs['edge_attr'])
        aggr_out = self.aggregate(message_out, row, size)
        out = self.update(aggr_out)
        return out
    
    def message(self, x_j, edge_attr=None, **kwargs):
        if edge_attr is not None:
            return x_j + edge_attr
        return x_j
        
    def aggregate(self, messages, row, size):
        num_nodes = size[0] if size is not None else int(row.max()) + 1
        out = torch.zeros(num_nodes, messages.size(-1), device=messages.device)
        if self.aggr == 'add':
            out.index_add_(0, row, messages)
        elif self.aggr == 'mean':
            count = torch.bincount(row, minlength=num_nodes).clamp(min=1)
            out.index_add_(0, row, messages)
            out /= count.unsqueeze(-1)
        return out
...
class LoRPrompterDown(nn.Module):
    def __init__(self,
        ...
        self.project_down = nn.Linear(self.embed_dims, self.rank_dims, bias=True)
        self.init_weights()
...
class LoRPrompterUp(nn.Module):
    def __init__(self,
        ...
        if not linear_transfrom:
            self.activate = nn.GELU()
        self.project_up = nn.Linear(self.rank_dims, self.embed_dims, bias=True)
        self.init_weights()
...
class GINConv(CustomMessagePassing):
    """
...
class GINConv(CustomMessagePassing):
    ...
    def __init__(self, emb_dim, aggr="add", input_layer=False, rank_dim=32):
        super(GINConv, self).__init__()
        # multi-layer perceptron
        self.mlp = torch.nn.Sequential(torch.nn.Linear(emb_dim, 2 * emb_dim), torch.nn.ReLU(),
                                       torch.nn.Linear(2 * emb_dim, emb_dim))
        ...
        # Vision Graph Prompt modules
        self.rank_dim = rank_dim
        self.graph_prompt = nn.Parameter(torch.zeros([rank_dim, emb_dim]))
        self.node_prompter = LoRPrompterDown(emb_dim, rank_dims=rank_dim, linear_transfrom=False)
        self.edge_prompt = LoRPrompterUp(embed_dims=emb_dim, rank_dims=rank_dim, linear_transfrom=True)
    ...
    def forward(self, x, edge_index, edge_attr, prompt=None):
        ...
        low_rank_x = self.node_prompter(x) #[B, r]
        res_node_prompts = low_rank_x@self.graph_prompt
        x = x*0.8+res_node_prompts*0.2
        # (center_index, neighbor_index) = edge_index
        # low_rank_neighbors = low_rank_x[neighbor_index]
        # edge_prompts = self.edge_prompt(low_rank_neighbors)
        # add
        # x.index_add_(0, center_index, edge_prompts)
        # mean
        # num_nodes = int(center_index.max()) + 1
        # count = torch.bincount(center_index, minlength=num_nodes).clamp(min=1)
        # mean_edge_prompts = torch.zeros_like(x, device=x.device)
        # mean_edge_prompts.index_add_(0, center_index, edge_prompts)
        # mean_edge_prompts = mean_edge_prompts/count.unsqueeze(-1)
        # x = x*0.9 + mean_edge_prompts*0.1
...
class GNN(torch.nn.Module):
    ...
    def forward(self, *argv):
        ...
        # for i, f in enumerate(h_list):
        #     np.save(f.detach().clone().numpy(),f'depth-{i}')
...
class GNN_visiongraphprompt(torch.nn.Module):
    """
    ...
    def __init__(self, num_layer, emb_dim, num_tasks, JK="last", drop_ratio=0, graph_pooling="mean", gnn_type="gin",
                 final_linear=True, head_layer=1):
        super(GNN_visiongraphprompt, self).__init__()
        self.num_layer = num_layer
        self.drop_ratio = drop_ratio
        self.JK = JK
        self.emb_dim = emb_dim
        ...
        if self.JK == "concat":
            ...
            for i in range(head_layer - 1):
                self.graph_pred_linear.append(torch.nn.Linear(self.mult * (self.num_layer + 1) * self.emb_dim, self.mult * (self.num_layer + 1) * self.emb_dim))
                self.graph_pred_linear.append(torch.nn.ReLU())
            self.graph_pred_linear.append(torch.nn.Linear(self.mult * (self.num_layer + 1) * self.emb_dim, self.num_tasks))

        else:
            self.graph_pred_linear = torch.nn.ModuleList()
            for i in range(head_layer - 1):
                self.graph_pred_linear.append(torch.nn.Linear(self.mult * self.emb_dim, self.mult * self.emb_dim))
    ...
    def from_pretrained(self, model_file):
        ...
        incompatible_keys=self.gnn.load_state_dict(torch.load(model_file, map_location='cpu'), strict=False)
