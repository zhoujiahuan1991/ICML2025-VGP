...
def main():
    ...
    parser.add_argument('--num_layers', type=int, default = 2, help='A range of [1,2,3]-layer MLPs with equal width')
    ...
    #set up model
    # GNN_visiongraphprompt
    model = GNN_visiongraphprompt(args.num_layer, args.emb_dim, num_tasks, JK = args.JK, drop_ratio = args.dropout_ratio, graph_pooling = args.graph_pooling, gnn_type = args.gnn_type, head_layer = args.num_layers)
    ...
    with open(log_file, 'a+') as f:
        f.write(f'\n\n==================Method: {args.tuning_type} Dataset: {args.dataset}==================\n')
    ...
    best_epoch=np.array(test_acc_list).argmax()
    with open(log_file, 'a+') as f:
        f.write('best_epoch='+str(best_epoch+1)+'\n')
        f.write(args.dataset + ' ' + str(args.runseed) + ' ' + str(np.array(test_acc_list)[best_epoch]))
        f.write('\n')
