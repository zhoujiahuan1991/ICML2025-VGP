...
def gaussian_noise(shape: list, loc=0., scale=0.2, shell_radius=0.9):
    '''produce Shell Noise with Gaussian distribution to imitate noisy points from depth cameras'''
    gaussian = np.random.normal(loc, scale, shape)
    gaussian_direction = gaussian/np.linalg.norm(gaussian, ord=2, axis=-1, keepdims=True)
    gaussian = gaussian + gaussian_direction * shell_radius
    return gaussian

def lidar_noise(points, number=64, scale=1.3, low=1.02):
    noise_idx = np.random.choice(points.shape[1],number)
    noise_factor = np.random.uniform(low, scale, size=number).astype(np.float32)[None, :, None]
    noise = points[:, noise_idx, :]*torch.from_numpy(noise_factor).to(points.device)
    return noise
...
def seprate_point_cloud(xyz, num_points, crop, fixed_points = None, padding_zeros = False, sample_points=1024, incomplete_shape=True):
    '''
    ...
    for points in xyz:
        ...
        if isinstance(crop,list):
            INPUT.append(fps(input_data,sample_points)[0])
            CROP.append(fps(crop_data,sample_points)[0])
        else:
            if incomplete_shape and input_data.shape[1] > sample_points:
                INPUT.append(fps(input_data,sample_points)[0])
                CROP.append(fps(crop_data,sample_points)[0])
            else:
...
def summary_parameters(model, logger=None, in_detail=True):
    if in_detail:
