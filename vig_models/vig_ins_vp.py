...
class Grapher(nn.Module):
    ...
    def __init__(self, in_channels, kernel_size=9, dilation=1, conv='edge', act='relu', norm=nn.LayerNorm,
                 bias=True,  stochastic=False, epsilon=0.0, r=1, n=196, drop_path=0.0, relative_pos=False, add_prompts=False, prompt_length=14, args={}):
    ...
    def forward(self, x, instance_prompts):
        ...
        if self.add_prompts:
            ...
            token_prompts = self.prompt_dropout(token_prompts.expand([B,-1,-1,-1]))
            token_prompts = token_prompts + instance_prompts
...
class DeepGCN(torch.nn.Module):
    def __init__(self, opt):
        ...
        self.InsTokenPrompt = TokenPrompt(self.args, channels)
        ...
        if opt.use_dilation:
            self.backbone = Seq(*[Seq(
                                      Grapher(channels, num_knn[i], min(i // 4 + 1, max_dilation), conv, act, norm, bias, stochastic, epsilon, 1, drop_path=dpr[i], add_prompts=True, prompt_length=14, args=self.args),
                                      FFN(channels, channels * 4, act=act, drop_path=dpr[i])
                                     ) for i in range(self.n_blocks)])
        else:
            self.backbone = Seq(*[Seq(
                                      Grapher(channels, num_knn[i], 1, conv, act, norm, bias, stochastic, epsilon, 1, drop_path=dpr[i], add_prompts=True, prompt_length=14, args=self.args),
                                      FFN(channels, channels * 4, act=act, drop_path=dpr[i])
                                     ) for i in range(self.n_blocks)])
...
@register_model
def vig_ins_vp_ti_224_gelu(**kwargs):
    class OptInit:
...
@register_model
def vig_ins_vp_s_224_gelu(**kwargs):
    class OptInit:
        def __init__(self, num_classes=1000, drop_path_rate=0.0, drop_rate=0.0, num_knn=9, **kwargs):
            self.k = num_knn # neighbor num (default:9)
            self.conv = 'mr' # graph conv layer {edge, mr}
...
@register_model
def vig_ins_vp_b_224_gelu(**kwargs):
    class OptInit:
        def __init__(self, num_classes=1000, drop_path_rate=0.0, drop_rate=0.0, num_knn=9, **kwargs):
            self.k = num_knn # neighbor num (default:9)
            self.conv = 'mr' # graph conv layer {edge, mr}
