...
import pyramid_vig_models.pyramid_vig as pyramid_vig
import vig_models.vig as vig
...
parser.add_argument('--data', metavar='DIR', default="./data/VTAB",
                    help='path to dataset')
parser.add_argument('--base_dir', default="./data",
                    help='base directory for datasets')
parser.add_argument('--dataset', type=str, default="cifar100")
parser.add_argument('--crop_size', type=int, default=224)
parser.add_argument('--exp-name', type=str, default='vig-finetune')
parser.add_argument('--model', default='vig_s_224_gelu', type=str, metavar='MODEL',
                    help='Name of model to train')
parser.add_argument("--pretrain_path", default='pretrained_bases/vig_s_80.6.pth', type=str)
...
def main():
    ...
    if args.distributed:
        ...
        if has_apex and use_amp != 'native':
            ...
            model = TorchDDP(model, delay_allreduce=True)
        else:
            if args.local_rank == 0:
    ...
    dataset_train = data_loader.construct_train_dataset(args)
    dataset_eval = data_loader.construct_val_dataset(args)
    dataset_test =data_loader.construct_test_dataset(args)
    print("Nums of classes: {}".format(len(dataset_train.class_to_idx)))
    print("Sample nums: [train]-{}, [val]-{}, [test]-{}".format(len(dataset_train), len(dataset_eval), len(dataset_test)))
    ...
    if args.evaluate:
        eval_metrics = validate(model, dataset_eval, validate_loss_fn, args, amp_autocast=amp_autocast)
        print(eval_metrics)
        return
    ...
    if args.local_rank == 0:
        ...
        output_dir = get_outdir(output_base, args.dataset, args.exp_name, exp_tag)
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            eval_metrics = validate(model, eval_loader, validate_loss_fn, args, amp_autocast=amp_autocast)

            if model_ema is not None and not args.model_ema_force_cpu:
                if args.distributed and args.dist_bn in ('broadcast', 'reduce'):
                    distribute_bn(model_ema, args.world_size, args.dist_bn == 'reduce')
                ema_eval_metrics = validate(model_ema.ema, eval_loader, validate_loss_fn, args, amp_autocast=amp_autocast, log_suffix='[EMA]')
